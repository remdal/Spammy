...
: m_indexCount(0), m_gridEdgeIndexCount(0)
{
    // Init grid
    ...
    // Create GPU buffers
    m_voxelBuffer = device->newBuffer(m_voxels.size() * sizeof(Voxel),
                                        MTL::ResourceStorageModeShared);
    m_uniformsBuffer = device->newBuffer(sizeof(GridUniforms),
                                           MTL::ResourceStorageModeShared);
    
    // Allocate mesh buffers (initial size)
    m_meshVertexBuffer = device->newBuffer(1024 * 1024 * sizeof(Vertex),
                                            MTL::ResourceStorageModeShared);
    m_meshIndexBuffer = device->newBuffer(1024 * 1024 * sizeof(uint32_t),
                                           MTL::ResourceStorageModeShared);
...
void RMDLGrid::createPipelines()
{
    ...
    // Depth state for grid edges
    MTL::DepthStencilDescriptor* depthDesc = MTL::DepthStencilDescriptor::alloc()->init();
    depthDesc->setDepthCompareFunction(MTL::CompareFunctionLess);
    depthDesc->setDepthWriteEnabled(false);  // Don't write to depth for grid lines
...
void RMDLGrid::renderGridEdges(MTL::RenderCommandEncoder* encoder, const simd::float4x4& viewProjection) {
    ...
    simd::float4 gridColor = {0.0f, 1.0f, 1.0f, 0.5f};  // Cyan semi-transparent
...
void RMDLGrid::renderChunks(MTL::RenderCommandEncoder* encoder, const simd::float4x4& viewProjection) {
    if (!m_voxelPipeline || m_indexCount == 0) return;

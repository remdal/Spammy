...
std::string PhaseAudio::loadMonoSound(const std::string &assetSearchPath, const std::string &assetName)
{
    NSError* pError = nullptr;
    PHASEEngine* engine = (__bridge PHASEEngine *)_phaseEngine;
    NSString* urlString = [NSString stringWithFormat:@"%s/%s", assetSearchPath.c_str(), assetName.c_str()];
    NSURL* url = [NSURL URLWithString:urlString];
    NSString* assetIdentifier = [NSString stringWithUTF8String:assetName.c_str()];
    PHASESoundAsset* soundAsset = [engine.assetRegistry registerSoundAssetAtURL:url identifier:assetIdentifier assetType:PHASEAssetTypeResident channelLayout:nil normalizationMode:PHASENormalizationModeDynamic error:&pError];
    AVAudioChannelLayout* channelLayout = [[AVAudioChannelLayout alloc] initWithLayoutTag:kAudioChannelLayoutTag_Mono];
    PHASEChannelMixerDefinition* channelMixerDefinition = [[PHASEChannelMixerDefinition alloc] initWithChannelLayout:channelLayout];
    PHASESamplerNodeDefinition* samplerNodeDefinition = [[PHASESamplerNodeDefinition alloc] initWithSoundAssetIdentifier:[NSString stringWithUTF8String:assetName.c_str()] mixerDefinition:channelMixerDefinition];

    samplerNodeDefinition.playbackMode = PHASEPlaybackModeLooping;
    ...
    PHASESoundEventNodeAsset* soundEventAsset = [engine.assetRegistry registerSoundEventAssetWithRootNode:samplerNodeDefinition identifier:eventIdentifier error:&pError];

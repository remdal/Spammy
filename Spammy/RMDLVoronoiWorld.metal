...
float perlinNoise3D_GPU(float3 p)
{
    ...
    float c000 = sin(float(hash_gpu(pi.x, pi.y, pi.z) % 360u) * 0.017453292);
...
//    auto grad = [](int3 p) -> float {
//        uint h = hash_gpu(p.x, p.y, p.z);
//        float angle = float(h % 360u) * 0.017453292; // deg to rad
//        return sin(angle);
//    };
//    
//    float c000 = grad(pi + int3(0,0,0));
//    float c100 = grad(pi + int3(1,0,0));
//    float c010 = grad(pi + int3(0,1,0));
//    float c110 = grad(pi + int3(1,1,0));
//    float c001 = grad(pi + int3(0,0,1));
//    float c101 = grad(pi + int3(1,0,1));
//    float c011 = grad(pi + int3(0,1,1));
//    float c111 = grad(pi + int3(1,1,1));
//    
